...
void CPlaylist::Erase(int nIndex)
{
	vector<string_t>::iterator Iter = m_vctPath.begin();
	if(m_vctPath.size() >= nIndex)
		m_vctPath.erase(Iter + nIndex);
}
...
int CPlaylist::FindFile(const string_t &strPath)
{
	vector<string_t>::iterator it = find(m_vctPath.begin(), m_vctPath.end(), strPath);
	if(it == m_vctPath.end()) 
		return -1; 
	return (int)(it-m_vctPath.begin());
}
